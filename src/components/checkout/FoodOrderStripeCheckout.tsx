...
interface FoodOrderStripeCheckoutProps {
...
export function FoodOrderStripeCheckout(props: FoodOrderStripeCheckoutProps) {
  const [clientSecret, setClientSecret] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    // Create payment intent for food order
    const createPaymentIntent = async () => {
      try {
        console.log("[FoodOrderStripeCheckout] Creating payment intent...", {
          total: props.total,
          orderId: props.orderId,
        });

        const response = await fetch("/api/stripe/create-food-order-payment", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            amount: props.total, // Already in cents
            orderId: props.orderId,
        ...
        if (!response.ok) {
          console.error("[FoodOrderStripeCheckout] API error:", data);
  ...
  const elementsOptions: StripeElementsOptions = {
    ...
    appearance: {
      theme: "stripe",
      variables: {
        colorPrimary: "#1e9df1", // SteppersLife primary blue
